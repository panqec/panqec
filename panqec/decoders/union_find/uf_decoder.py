...
class UnionFindDecoder(BaseDecoder):
    ...
    def decode(self, syndrome: np.ndarray) -> np.ndarray:
        ...
        # note how we create different variables for x & z syndromes. But to make the code compatible with both errors --> change to 1 variable
        # OR --> put peeling in a seperate file as a function, and call the clustering + peeling functions twice in this file
        ...
        syndromes_z_index = np.where(syndromes_z == 1)[0] # this returns an array of indices of stabilizers that are syndromes
        #syndromes_x_index = np.where(syndromes_x == 1) [0]
        ...
        ### clustering/syndrome validation stage. Outputs clusters
        # output = [1:(s[0,1,2], q[3,4,5]), 2:(s[22,33,44], q[34,46,57]), ...], a list of tuples of stabilizers and qubit indices.
        # each tuple represents a cluster.
        output = clustering(syndromes_z, Hz)
        print(f"The output of clustering is: {output}")
        print("\n")       
        ...
        for cluster in output:           
            ...
            for vertex in vertices_i: 
                ...
                if vertex in syndromes_z_index:
                    sig_i.append(vertex)
        ...
        def find_tree(g):
                ...
                print(f"initial dictionary parent: {visited}")
                ...
                while len(q): # while the que is not empty
                    ...
                    print(f"* Now checking out neighbours of \"current_node\" *")
                    print(f"neighbours of current_node({current_node}): {find_neighbours(tree,current_node)}")
                    neighbour_index = 0
        ...
        for i,tree in enumerate(forest):
            ...
            while len(tree.keys())>1: # while tree dictionary is not empty. tree = {0:[]} is considered empty for us!!
                ...
                if len(tree[v])==1:
                    leaves.append(v)
                    print("* v is now a leaf *")
                    print(f"leaves after (adding v): {leaves}")
                else:
                    print("v is NOT a leaf")
                    print(f"leaves (without adding v): {leaves}")

                #if len(tree[u])==0: --> remove if condition of while loop is appropriate
                    #tree.pop(u)
        ...
        ## Now proccess A_ to find correction_x of the bsf ##

        ## Processing A_ to extract the edge/qubit indices in panqec --> better ways: Hz * Hz^T  or use panqec coordinates ( the average of the coordinates of v1 & v2, this can be a function) 

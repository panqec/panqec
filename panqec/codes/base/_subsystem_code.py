...
class SubsystemCode(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self._gauge_coordinates: List[Tuple] = []
    ...
    def gauge_index(self) -> Dict[Tuple, int]:
        ...
        if len(self._gauge_index) == 0:
            self._gauge_index = {
                loc: i for i, loc in enumerate(self.gauge_coordinates)
            }
    ...
    def is_gauge(self, location: Tuple, gauge_type: Optional[str] = None):
        """Returns whether a given location in the coordinate system
        corresponds to a stabilizer or not
        """
        _is_gauge = (
            (location in self.gauge_index) and
            (gauge_type is None or self.gauge_type(location) == gauge_type)
        )
    ...
    def get_gauge_coordinates(self) -> List[Tuple]:
        """List of coordinates for gauge operators, in a coordinate system
        that should contain both the qubits and the stabilizers.
        This function is used to set the attributes
        `self.gauge_coordinates` and `self.gauge_index`.
        """
    ...
    @abstractmethod
    def gauge_type(self, location: Tuple) -> str:
        """ Returns the type of a gauge operator sitting at a given location.
        E.g. 'horizontal' or 'vertical' in the Bacon-Shor code
        """
    ...
    def get_stabilizer(self, location: Tuple) -> Operator:
        ...
        gauge_ops = self.get_stabilizer_gauge_operators(location)

        stabilizer = {}
        for gauge_coord in gauge_ops:
            gauge_op = self.get_gauge_operator(gauge_coord)

            # TODO: replace by actual operator multiplication
            stabilizer.update(gauge_op)

        return stabilizer

    @abstractmethod
    def get_gauge_operator(self, gauge_coordinates: Tuple) -> Operator:
        """ Returns a gauge operator, formatted as dictionary that assigns
        a Pauli operator ('X', 'Y' or 'Z') to each qubit location in the
        support of the gauge operator.

        For example, for a gauge operator in the [[4,1,2]] code, we could have
        `get_gauge operator((1,1)) -> {(0,1): 'X', (2, 1): 'X'}`

        Parameters
        ----------
        gauge_coordinates: Tuple
            Coordinates of the gauge operator

        Returns
        -------
        gauge_op: Dict[Tuple, str]
            Dictionary that assigns a Pauli operator ('X', 'Y' or 'Z') to each
            qubit location in the support of the gauge operator
        """
    ...
    def qubit_representation(self,
        ...
        rotated_picture: bool
            For codes that have a rotated picture, can be used to
            differentiate the two types visualizations
        json_file: str
            File with the initial configuration for the code
    ...
    def gauge_representation(self, gauge_coord: Tuple) -> Dict:
        representation = {'location': gauge_coord}
